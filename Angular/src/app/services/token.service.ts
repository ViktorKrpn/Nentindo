...
import { Token } from '../types/token/index';
import { LocalStorageService } from './local-storage.service';
import { JwtToken } from '../types/token/index';
import { currentTimestamp, filterObject } from '../types/token/index';
@Injectable({
    providedIn: 'root',
})
export class TokenService implements OnDestroy {
    ...
    private save(token?: Token): void {
        ...
        if (!token) {
            ...
            this.store.set(this.key, filterObject(value as any));
        }
    }
}